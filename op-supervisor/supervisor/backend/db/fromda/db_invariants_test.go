...
func invariantNumberIncrement(prev, current LinkEntry) error {
	...
	if derivedSame && sourceSame {
		return fmt.Errorf("expected at least either source or derived to increment, but both have same number")
	}
	...
	if !(sourceSame || sourceIncrement) {
		return fmt.Errorf("expected source to either stay the same or increment, got prev %s current %s", prev.source, current.source)
	}
	return nil
}
