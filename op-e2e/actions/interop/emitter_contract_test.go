...
func includeTxOnChain(t helpers.Testing, actors *InteropActors, chain *Chain, tx *types.Transaction, sender common.Address) {
	...
	fmt.Println("Created L2 block on chain", chain.ChainID)
	fmt.Println("Unsafe head:", status.UnsafeL2.ID().Number)
	fmt.Println("CrossUnsafe head:", status.CrossUnsafeL2.ID().Number)
	fmt.Println("LocalSafe head:", status.LocalSafeL2.ID().Number)
	fmt.Println("Safe head:", status.SafeL2.ID().Number)
	fmt.Println("Finalized head:", status.FinalizedL2.ID().Number)
	fmt.Println("FinalizedL1:", status.FinalizedL1.ID().Number)
	fmt.Println(status)
}

func assertHeads(t helpers.Testing, chain *Chain, localUnsafe, localSafe, crossUnsafe uint64) {
	status := chain.Sequencer.SyncStatus()
	require.Equal(t, localUnsafe, status.UnsafeL2.ID().Number)
