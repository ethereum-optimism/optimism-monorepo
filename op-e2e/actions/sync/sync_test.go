...
func TestELSyncTransitionsToCLSyncAfterNodeRestart(gt *testing.T) {
	...
	verifier = actionsHelpers.NewL2Verifier(t, captureLog, miner.L1Client(t, sd.RollupCfg), miner.BlobStore(),
		altda.Disabled, verifier.Eng, sd.RollupCfg, sd.L2Cfg.Config.Optimism,
		&sync.Config{SyncMode: sync.ELSync}, actionsHelpers.DefaultVerifierCfg().SafeHeadListener)
...
func TestForcedELSyncCLAfterNodeRestart(gt *testing.T) {
	...
	verifier2 := actionsHelpers.NewL2Verifier(t, captureLog, miner.L1Client(t, sd.RollupCfg), miner.BlobStore(),
		altda.Disabled, verifier.Eng, sd.RollupCfg, sd.L2Cfg.Config.Optimism,
		&sync.Config{SyncMode: sync.ELSync, SupportsPostFinalizationELSync: true},
		actionsHelpers.DefaultVerifierCfg().SafeHeadListener)
