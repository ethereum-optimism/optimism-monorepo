...
func (d *consolidateCheckDeps) Check(
	...
	for _, receipt := range receipts {
		for i, log := range receipt.Logs {
			if current+uint32(i) == logIdx {
				msgHash := logToMessageHash(log)
				if msgHash != logHash {
					return supervisortypes.BlockSeal{}, fmt.Errorf("payload hash mismatch: %s != %s: %w", msgHash, logHash, supervisortypes.ErrConflict)
				} else if block.Time() != timestamp {
					return supervisortypes.BlockSeal{}, fmt.Errorf("block timestamp mismatch: %d != %d: %w", block.Time(), timestamp, supervisortypes.ErrConflict)
				} else {
