...
if ! { git diff origin/ctb/rm-beta-tags...HEAD --name-only; git diff --name-only; git diff --cached --name-only; } | grep -q "$SEMVER_LOCK"; then
    echo "No changes detected in semver-lock.json"
    exit 0
fi

# Get the upstream semver-lock.json.
if ! git show origin/ctb/rm-beta-tags:packages/contracts-bedrock/snapshots/semver-lock.json > "$temp_dir/upstream_semver_lock.json" 2>/dev/null; then
      echo "❌ Error: Could not find semver-lock.json in the snapshots/ directory of ctb/rm-beta-tags branch"
      exit 1
fi
...
for contract in $changed_contracts; do
    ...
    git show origin/ctb/rm-beta-tags:packages/contracts-bedrock/"$contract" > "$old_source_file" 2>/dev/null || true
