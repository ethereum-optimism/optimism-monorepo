...
library Encoding {
    ...
    function encodeSuperRoot(Types.SuperRootProof memory _superRootProof) internal pure returns (bytes memory) {
        ...
        bytes memory encoded = abi.encodePacked(uint8(0x01), bytes8(_superRootProof.timestamp));
        ...
        for (uint256 i = 0; i < _superRootProof.outputRoots.length; i++) {
            ...
            encoded = abi.encodePacked(encoded, outputRoot.chainId, outputRoot.root);
        }
