...
contract AnchorStateRegistry is Initializable, ISemver {
    ...
    function isGameRespected(IDisputeGame _game) public view returns (bool) {
        try _game.wasRespectedGameTypeWhenCreated() returns (bool respected_) {
            return respected_;
        } catch {
            // Legacy games do not have the wasRespectedGameTypeWhenCreated function.
            return false;
        }
    }
    ...
    function isGameAirgapped(IDisputeGame _game) public view returns (bool) {
        return block.timestamp - _game.resolvedAt().raw() > DISPUTE_GAME_FINALITY_DELAY_SECONDS;
    }
