...
// Target contract
import {L2ToL2CrossDomainMessenger, NotEntered, MessageDestinationSameChain, IdOriginNotL2ToL2CrossDomainMessenger, EventPayloadNotSentMessage, MessageDestinationNotRelayChain, MessageTargetCrossL2Inbox, MessageTargetL2ToL2CrossDomainMessenger, MessageAlreadyRelayed, ReentrantCall, TargetCallFailed, IDependencySet, InvalidChainId} from "src/L2/L2ToL2CrossDomainMessenger.sol";
...
contract L2ToL2CrossDomainMessengerWithModifiableTransientStorage is
...
contract L2ToL2CrossDomainMessengerTest is Test {
    address internal foundryVMAddress =
    ...
    function setUp() public {
        ...
        vm.etch(
            ...
            address(
                new L2ToL2CrossDomainMessengerWithModifiableTransientStorage()
            ).code
        );
        l2ToL2CrossDomainMessenger = L2ToL2CrossDomainMessengerWithModifiableTransientStorage(
            Predeploys.L2_TO_L2_CROSS_DOMAIN_MESSENGER
        );
    }
    ...
    function testFuzz_sendMessage_succeeds(
        ...
        bytes32 msgHash = l2ToL2CrossDomainMessenger.sendMessage(
            _destination,
            _target,
            _message
        );
    ...
    function testFuzz_sendMessage_nonPayable_reverts(
        ...
        (bool success, ) = address(l2ToL2CrossDomainMessenger).call{
            value: _value
        }(
            abi.encodeCall(
    ...
    function testFuzz_sendMessage_targetCrossL2Inbox_reverts(
        uint256 _destination,
        bytes calldata _message
    ) external {
    ...
    function testFuzz_relayMessage_eventPayloadNotSentMessage_reverts(
        ...
        bytes memory sentMessage = abi.encode(
            L2ToL2CrossDomainMessenger.RelayedMessage.selector,
            _source,
            _nonce,
            _msgHash
        );
    ...
    function testFuzz_relayMessage_metadataStore_succeeds(
        ...
        bytes32 msgHash = keccak256(
            abi.encode(block.chainid, _source, _nonce, _sender, target, message)
        );
    ...
    function testFuzz_relayMessage_returnData_succeeds(
        ...
        vm.mockCall({
            callee: _target,
            data: message,
            returnData: _mockedReturnData
        });

        hoax(Predeploys.L2_TO_L2_CROSS_DOMAIN_MESSENGER, _value);
        bytes memory returnData = l2ToL2CrossDomainMessenger.relayMessage{
            value: _value
        }(id, sentMessage);
    ...
    function testFuzz_relayMessage_reentrant_reverts(
        ...
        address target = address(this);
        bytes memory message = abi.encodeCall(
            this.mockTargetReentrant,
            (_source2, _nonce, _sender2)
        );
    ...
    function testFuzz_crossDomainMessageSender_succeeds(
        address _sender
    ) external {
    ...
    function testFuzz_crossDomainMessageSource_succeeds(
        uint256 _source
    ) external {
        // Set `entered` to non-zero value to prevent NotEntered revert
        l2ToL2CrossDomainMessenger.setEntered(1);
    ...
    function testFuzz_crossDomainMessageContext_succeeds(
        ...
        (
            address crossDomainContextSender,
            uint256 crossDomainContextSource
