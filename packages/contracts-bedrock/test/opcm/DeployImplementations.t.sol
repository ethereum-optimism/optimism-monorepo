...
contract DeployImplementations_Test is Test {
    ...
    function test_deploy_atNonExistentRelease_reverts() public {
        ...
        dio.set(dio.l1ERC721BridgeImpl.selector, l1ERC721BridgeImpl);
        address optimismPortalImpl = makeAddr("optimismPortalImpl");
        dio.set(dio.optimismPortalImpl.selector, optimismPortalImpl);
        address systemConfigImpl = makeAddr("systemConfigImpl");
        dio.set(dio.systemConfigImpl.selector, systemConfigImpl);
        address optimismMintableERC20FactoryImpl = makeAddr("optimismMintableERC20FactoryImpl");
        dio.set(dio.optimismMintableERC20FactoryImpl.selector, optimismMintableERC20FactoryImpl);
        address l1CrossDomainMessengerImpl = makeAddr("l1CrossDomainMessengerImpl");
        dio.set(dio.l1CrossDomainMessengerImpl.selector, l1CrossDomainMessengerImpl);
        address l1StandardBridgeImpl = makeAddr("l1StandardBridgeImpl");
        dio.set(dio.l1StandardBridgeImpl.selector, l1StandardBridgeImpl);
        address disputeGameFactoryImpl = makeAddr("disputeGameFactoryImpl");
        dio.set(dio.disputeGameFactoryImpl.selector, disputeGameFactoryImpl);
        address delayedWETHImpl = makeAddr("delayedWETHImpl");
        dio.set(dio.delayedWETHImpl.selector, delayedWETHImpl);
        address mipsSingleton = makeAddr("mipsSingleton");
        dio.set(dio.mipsSingleton.selector, mipsSingleton);

        vm.etch(address(superchainConfigProxy), hex"01");
        vm.etch(address(protocolVersionsProxy), hex"01");
        vm.etch(l1ERC721BridgeImpl, hex"01");
        vm.etch(optimismPortalImpl, hex"01");
        vm.etch(systemConfigImpl, hex"01");
        vm.etch(optimismMintableERC20FactoryImpl, hex"01");
        vm.etch(l1CrossDomainMessengerImpl, hex"01");
        vm.etch(l1StandardBridgeImpl, hex"01");
        vm.etch(disputeGameFactoryImpl, hex"01");
        vm.etch(delayedWETHImpl, hex"01");
        vm.etch(mipsSingleton, hex"01");
